...
const StoreCheckoutList = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/checkouts/byUserId/${id}`)
      .then((response) => {
      ...
      axios.get(`http://localhost:8080/checkouts/salesByUser/${id}`)
      .then((response) => {
    ...
    <div className="px-5 mt-5">
      ...
      <div className="mb-3">
        <div className="card p-3">
          ...
          <p className="card-subtitle">Total Purchase as of {moment().format("dddd, MMMM Do YYYY, h:mm:ss a")}</p>
            ...
            <div className="col-sm-4 mb-3" key={checkout.id}>
              ...
              <img
                ...
                <div className="card-body">
                  <h5 className="card-title">{checkout.firstName}, {checkout.lastName}</h5>
                  <p className="card-subtitle mb-3">{checkout.product.name}</p>
                  <p className="card-subtitle">{checkout.street}, {checkout.zipcode}</p>
                  <p className="card-subtitle">{checkout.email}</p>
                  <p className="card-subtitle">{checkout.checkoutuser.username}</p>
